...
<div class="width_adjust" style="margin: 0 auto;">
  <br><br>
    <div class="container">     
      <div>
        <p style="color: red;">※旧システムからの移行段階のため、多数の不備がある点をご容赦ください。</p>
      </div>
      <div id = "" style="display: block;">
        <img src = 'icon/computer_work.png' alt="computer"><span style="font-size:32px;font-weight:bold;color:#30b279;">過去問</span>にチャレンジ
      </div>
        ...
        <form action="{{ route(('select_menu')) }}" method="post">
          ...
          <div class="card_outer">
            <div style="height: 150px; display: flex;">
              <div class="rectant" style="display: block; width:15px;height: 65px; background-color: green; margin-top: 15px;margin-left: 15px;">
              </div>
              <div class="" style="width: 250px; height: 65px; background-color: rgb(223, 226, 234); margin-top: 15px;">

                <div style="display: flex; height: 65px; align-items: center; justify-content: center;">
                  {{--
            ...
            <div class="" style="height: 100px;">
              <table>
                ...
                <tr>
                  <td>
                    @if( null === $array_today_challengers[$index])
                      -
                    @elseif($array_today_challengers[$index] == 0)
                      -
                    @else
                      {{ $array_today_challengers[$index] }}
                    @endif
                    
                  </td>
                  <td>
                    @if( null === $array_total_challengers[$index])
                      -
                    @elseif($array_total_challengers[$index] == 0)
                      -
                    @else
                      {{ $array_total_challengers[$index] }}
                    @endif
                  </td>
                </tr>
              </table>
